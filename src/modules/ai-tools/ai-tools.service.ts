...
export class AIToolsService {
  ...
  buildDisciplineSelectionPrompt(disciplines: Discipline[]): string {
    ...
    return `Ты помощник для образовательной платформы Метаучебник. Твоя задача - проанализировать историю диалога пользователя с ИИ-ассистентом и определить, какие дисциплины из доступного списка наиболее релевантны для изучения вопросов пользователя.
...
- Убедись, что все UUID соответствуют предоставленным ID`;
  ...
  buildTopicSelectionPrompt(
    ...
    return `Ты помощник для образовательной платформы Метаучебник. Твоя задача - проанализировать историю диалога пользователя с ИИ-ассистентом и определить, какие темы из выбранных дисциплин наиболее релевантны для изучения вопросов пользователя.
...
5. НЕ возвращай пустой массив - всегда выбирай хотя бы несколько тем, если они есть в списке
  ...
  buildTrackRecommendationPrompt(
    ...
    return `Ты помощник для образовательной платформы Метаучебник. Твоя задача - проанализировать вопрос пользователя и определить, какие дисциплины и темы из доступного списка наиболее релевантны для изучения этого вопроса.
