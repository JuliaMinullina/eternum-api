...
const russianTopicsData = [
  ...
  },
  {
    ...
    ID: 636,
    TopicName:
      'Стили речи: разговорный, научный, публицистический, официально-деловой, художественный',
    DisciplineID: RUSSIAN_DISCIPLINE_ID,
    CreatedAt: new Date('2025-08-16T12:00:00Z'),
    UpdatedAt: new Date('2025-08-16T12:00:00Z'),
  },
  ...
  },
  {
    ...
    ID: 639,
    TopicName:
      'Создание письменных высказываний: сочинение, эссе, аргументация',
    DisciplineID: RUSSIAN_DISCIPLINE_ID,
    CreatedAt: new Date('2025-08-16T12:00:00Z'),
    UpdatedAt: new Date('2025-08-16T12:00:00Z'),
  },
...
async function bootstrap() {
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `⚠️  В базе уже есть ${existingCount} тем по русскому языку. Очищаю темы для этой дисциплины...`,
      );
    ...
    console.log(
      `✅ Успешно создано ${createdTopics.length} тем по русскому языку:`,
    );
    ...
    createdTopics.forEach((topic, index) => {
      ...
      // Категоризация тем
      if (
        topicName.includes('Язык и речь') ||
        topicName.includes('Нормы') ||
        topicName.includes('Культура речи')
      ) {
        categorizedTopics['Введение в русский язык'].push(topicName);
      } else if (
        topicName.includes('Фонетика') ||
        topicName.includes('орфоэпия') ||
        topicName.includes('Графика') ||
        topicName.includes('алфавит')
      ) {
        categorizedTopics['Фонетика и графика'].push(topicName);
      } else if (
        topicName.includes('Лексикология') ||
        topicName.includes('Синонимия') ||
        topicName.includes('антонимия') ||
        topicName.includes('омонимия') ||
        topicName.includes('паронимия') ||
        topicName.includes('Фразеология')
      ) {
        categorizedTopics['Лексикология и фразеология'].push(topicName);
      } else if (
        topicName.includes('Морфемика') ||
        topicName.includes('Словообразование')
      ) {
        categorizedTopics['Морфемика и словообразование'].push(topicName);
      } else if (
        topicName.includes('Морфология') ||
        topicName.includes('существительное') ||
        topicName.includes('прилагательное') ||
        topicName.includes('числительное') ||
        topicName.includes('Местоимение') ||
        topicName.includes('Глагол') ||
        topicName.includes('Причастие') ||
        topicName.includes('Деепричастие') ||
        topicName.includes('Наречие') ||
        topicName.includes('Служебные') ||
        topicName.includes('Междометия')
      ) {
        categorizedTopics['Морфология'].push(topicName);
      } else if (
        topicName.includes('Орфография') ||
        topicName.includes('Правописание')
      ) {
        categorizedTopics['Орфография'].push(topicName);
      } else if (
        topicName.includes('Синтаксис') ||
        topicName.includes('предложение') ||
        topicName.includes('члены') ||
        topicName.includes('Однородные') ||
        topicName.includes('Обособленные') ||
        topicName.includes('Вводные') ||
        topicName.includes('Сложные') ||
        topicName.includes('Сложносочинённое') ||
        topicName.includes('Сложноподчинённое') ||
        topicName.includes('Бессоюзное') ||
        topicName.includes('Прямая речь') ||
        topicName.includes('диалог')
      ) {
        ...
        topicName.includes('Текст') ||
        topicName.includes('Типы речи') ||
        topicName.includes('Стили речи') ||
        topicName.includes('межфразной') ||
        topicName.includes('Редактирование') ||
        topicName.includes('Создание') ||
        topicName.includes('сочинение') ||
        topicName.includes('эссе')
      ) {
    ...
    console.log(
      `🔗 Привязано к дисциплине: "Русский язык" (${RUSSIAN_DISCIPLINE_ID})`,
    );
  } catch (error) {
