...
const informaticsTopicsData = [
  ...
  },
  {
    ...
    ID: 707,
    TopicName:
      'Базовые структуры данных: массив, список, стек, очередь (обзор)',
    DisciplineID: INFORMATICS_DISCIPLINE_ID,
    CreatedAt: new Date('2025-08-16T12:00:00Z'),
    UpdatedAt: new Date('2025-08-16T12:00:00Z'),
  },
  ...
  },
  {
    ...
    ID: 728,
    TopicName:
      'Цифровые проекты: постановка задач, Agile/канбан, командная работа',
    DisciplineID: INFORMATICS_DISCIPLINE_ID,
    CreatedAt: new Date('2025-08-16T12:00:00Z'),
    UpdatedAt: new Date('2025-08-16T12:00:00Z'),
  },
...
async function bootstrap() {
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `⚠️  В базе уже есть ${existingCount} тем по информатике. Очищаю темы для этой дисциплины...`,
      );
    ...
    console.log(
      `✅ Успешно создано ${createdTopics.length} тем по информатике:`,
    );
    ...
    createdTopics.forEach((topic, index) => {
      ...
      // Категоризация тем
      if (
        topicName.includes('Информатика как наука') ||
        topicName.includes('Системы счисления') ||
        topicName.includes('кодирование информации')
      ) {
        categorizedTopics['Введение в информатику'].push(topicName);
      } else if (
        topicName.includes('Архитектура компьютера') ||
        topicName.includes('Операционные системы') ||
        topicName.includes('Компьютерные сети') ||
        topicName.includes('Интернет') ||
        topicName.includes('Информационная безопасность')
      ) {
        categorizedTopics['Компьютерные системы'].push(topicName);
      } else if (
        topicName.includes('Алгоритмы') ||
        topicName.includes('структуры данных') ||
        topicName.includes('Сложность алгоритмов') ||
        topicName.includes('исполнители')
      ) {
        categorizedTopics['Алгоритмы и структуры данных'].push(topicName);
      } else if (
        topicName.includes('Программирование') ||
        topicName.includes('переменные') ||
        topicName.includes('Функции') ||
        topicName.includes('модули') ||
        topicName.includes('Объектно-ориентированное') ||
        topicName.includes('файлами') ||
        topicName.includes('Отладка') ||
        topicName.includes('тестирование') ||
        topicName.includes('Git') ||
        topicName.includes('контроль версий')
      ) {
        categorizedTopics['Программирование'].push(topicName);
      } else if (
        topicName.includes('Базы данных') ||
        topicName.includes('SQL') ||
        topicName.includes('Проектирование данных') ||
        topicName.includes('нормализация')
      ) {
        categorizedTopics['Базы данных'].push(topicName);
      } else if (
        topicName.includes('Веб-разработка') ||
        topicName.includes('HTML') ||
        topicName.includes('CSS') ||
        topicName.includes('API') ||
        topicName.includes('REST') ||
        topicName.includes('GraphQL')
      ) {
        categorizedTopics['Веб-технологии'].push(topicName);
      } else if (
        topicName.includes('анализа данных') ||
        topicName.includes('таблицы') ||
        topicName.includes('визуализация') ||
        topicName.includes('статистика') ||
        topicName.includes('интеллект') ||
        topicName.includes('машинное обучение') ||
        topicName.includes('Генеративный ИИ')
      ) {
        categorizedTopics['Анализ данных и ИИ'].push(topicName);
      } else if (
        topicName.includes('Облачные') ||
        topicName.includes('Мобильная') ||
        topicName.includes('IoT') ||
        topicName.includes('Квантовые') ||
        ...
        topicName.includes('Карьера') ||
        topicName.includes('портфолио') ||
        topicName.includes('проекты') ||
        topicName.includes('Agile')
      ) {
        categorizedTopics['Этика и профессия'].push(topicName);
      } else if (
        topicName.includes('графика') ||
        topicName.includes('игровые') ||
        topicName.includes('мультимедиа') ||
        topicName.includes('Робототехника') ||
        topicName.includes('UX') ||
        topicName.includes('автоматизация')
      ) {
    ...
    console.log(
      `🔗 Привязано к дисциплине: "Информатика" (${INFORMATICS_DISCIPLINE_ID})`,
    );
  } catch (error) {
