...
async function bootstrap() {
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `⚠️  В базе уже есть ${existingCount} тем по химии. Очищаю темы для этой дисциплины...`,
      );
    ...
    createdTopics.forEach((topic, index) => {
      ...
      // Категоризация тем
      if (
        topicName.includes('Химия как наука') ||
        topicName.includes('Техника безопасности')
      ) {
        categorizedTopics['Введение в химию'].push(topicName);
      } else if (
        topicName.includes('Строение атома') ||
        topicName.includes('Периодический') ||
        topicName.includes('Количественные') ||
        topicName.includes('моль') ||
        topicName.includes('Авогадро')
      ) {
        categorizedTopics['Атомно-молекулярное учение'].push(topicName);
      } else if (
        topicName.includes('Химическая связь') ||
        topicName.includes('строение вещества') ||
        topicName.includes('Кристаллические') ||
        topicName.includes('аморфные') ||
        topicName.includes('ММВ')
      ) {
        categorizedTopics['Химическая связь и строение вещества'].push(
          topicName,
        );
      } else if (
        topicName.includes('Стехиометрия') ||
        topicName.includes('уравнениям реакций')
      ) {
        categorizedTopics['Количественные отношения'].push(topicName);
      } else if (
        (topicName.includes('реакций') && !topicName.includes('уравнениям')) ||
        topicName.includes('Термохимия') ||
        topicName.includes('Окислительно') ||
        topicName.includes('кинетика') ||
        topicName.includes('равновесие')
      ) {
        categorizedTopics['Химические реакции'].push(topicName);
      } else if (
        topicName.includes('Растворы') ||
        topicName.includes('Электролиты') ||
        topicName.includes('Кислоты') ||
        topicName.includes('основания') ||
        topicName.includes('соли') ||
        topicName.includes('pH') ||
        topicName.includes('гидролиз') ||
        topicName.includes('нейтрализация')
      ) {
        categorizedTopics['Растворы и электролиты'].push(topicName);
      } else if (
        topicName.includes('Оксиды') ||
        topicName.includes('Неметаллы') ||
        topicName.includes('Галогены') ||
        topicName.includes('Азот и фосфор') ||
        topicName.includes('Углерод и кремний') ||
        topicName.includes('Металлы') ||
        topicName.includes('активности') ||
        topicName.includes('Электрохимия')
      ) {
        categorizedTopics['Неорганическая химия'].push(topicName);
      } else if (
        topicName.includes('органическую') ||
        topicName.includes('Алканы') ||
        topicName.includes('Алкены') ||
        topicName.includes('алкины') ||
        topicName.includes('Ароматические') ||
        topicName.includes('Спирты') ||
        topicName.includes('фенолы') ||
        topicName.includes('Альдегиды') ||
        topicName.includes('кетоны') ||
        topicName.includes('Карбоновые') ||
        topicName.includes('эфиры') ||
        topicName.includes('Углеводы') ||
        topicName.includes('Амины') ||
        topicName.includes('белки') ||
        topicName.includes('Жиры') ||
        topicName.includes('мыла') ||
        ...
        topicName.includes('Коллоидные') ||
        topicName.includes('Экологическая') ||
        topicName.includes('зелёная') ||
        topicName.includes('современные материалы')
      ) {
    ...
    console.log(
      `🔗 Привязано к дисциплине: "Химия" (${CHEMISTRY_DISCIPLINE_ID})`,
    );
  } catch (error) {
