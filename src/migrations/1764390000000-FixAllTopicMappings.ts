...
export class FixAllTopicMappings1764390000000 implements MigrationInterface {
  name = 'FixAllTopicMappings1764390000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    console.log('🔧 Исправляю ВСЕ привязки тем к дисциплинам...');
    ...
    // Перемещаем темы по цепочке (в правильном порядке)
    // 1. Испанский ← МХК (темы МХК, которые сейчас в Испанском, нужно переместить обратно в МХК, а потом МХК темы в Испанский)
    // Сначала перемещаем темы МХК из Испанского в МХК
    ...
    // 2. МХК ← Культурология (темы Культурологии, которые сейчас в МХК)
    await queryRunner.query(`
      UPDATE "topics"
      SET "DisciplineID" = '${FINE_ARTS_CULTURE_DISCIPLINE_ID}'
    ...
    // 3. Культурология ← Психология (темы Психологии, которые сейчас в Культурологии)
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 4. Психология ← Философия (темы Философии, которые сейчас в Психологии)
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 5. Философия ← Испанский (темы Испанского, которые сейчас в Философии)
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 6. Теперь перемещаем темы МХК в Испанский
    await queryRunner.query(`
      UPDATE "topics"
      SET "DisciplineID" = '${SPANISH_DISCIPLINE_ID}'
      WHERE "TopicID" IN (${mhkTopicIds.map(id => `'${id}'`).join(', ')})
    `);
    console.log(`✅ Перемещено ${mhkTopicIds.length} тем из МХК в Испанский`);
