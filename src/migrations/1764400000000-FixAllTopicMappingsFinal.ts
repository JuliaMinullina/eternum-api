...
export class FixAllTopicMappingsFinal1764400000000
  implements MigrationInterface
{
  name = 'FixAllTopicMappingsFinal1764400000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    console.log('🔧 Финальное исправление всех привязок тем к дисциплинам...');

    // Получаем ID всех дисциплин
    ...
    // TopicID тем из МХК (должны быть в Испанском)
    const mhkTopicIds = [
    ...
    // TopicID тем из Культурологии (должны быть в МХК)
    ...
    // TopicID тем из Психологии (должны быть в Культурологии)
    ...
    // TopicID тем из Философии (должны быть в Психологии)
    ...
    // TopicID тем из Испанского (должны быть в Философии)
    ...
    // Делаем циклическую перестановку за один раз через временную дисциплину
    // Используем CASE для правильного распределения
    await queryRunner.query(`
      UPDATE "topics"
      SET "DisciplineID" = CASE
        WHEN "TopicID" IN (${mhkTopicIds.map(id => `'${id}'`).join(', ')}) THEN '${SPANISH_DISCIPLINE_ID}'
        WHEN "TopicID" IN (${culturologyTopicIds.map(id => `'${id}'`).join(', ')}) THEN '${FINE_ARTS_CULTURE_DISCIPLINE_ID}'
        WHEN "TopicID" IN (${psychologyTopicIds.map(id => `'${id}'`).join(', ')}) THEN '${CULTUROLOGY_DISCIPLINE_ID}'
        WHEN "TopicID" IN (${philosophyTopicIds.map(id => `'${id}'`).join(', ')}) THEN '${PSYCHOLOGY_DISCIPLINE_ID}'
        WHEN "TopicID" IN (${spanishTopicIds.map(id => `'${id}'`).join(', ')}) THEN '${PHILOSOPHY_DISCIPLINE_ID}'
        ELSE "DisciplineID"
      END
      WHERE "TopicID" IN (
        ${mhkTopicIds.map(id => `'${id}'`).join(', ')},
        ${culturologyTopicIds.map(id => `'${id}'`).join(', ')},
        ${psychologyTopicIds.map(id => `'${id}'`).join(', ')},
        ${philosophyTopicIds.map(id => `'${id}'`).join(', ')},
        ${spanishTopicIds.map(id => `'${id}'`).join(', ')}
      )
    `);

    console.log(`✅ Перемещено ${mhkTopicIds.length} тем МХК в Испанский`);
    console.log(`✅ Перемещено ${culturologyTopicIds.length} тем Культурологии в МХК`);
    console.log(`✅ Перемещено ${psychologyTopicIds.length} тем Психологии в Культурологию`);
    console.log(`✅ Перемещено ${philosophyTopicIds.length} тем Философии в Психологию`);
    console.log(`✅ Перемещено ${spanishTopicIds.length} тем Испанского в Философию`);
