...
export class FixAllTopicMappingsCorrectly1764410000000
  implements MigrationInterface
{
  name = 'FixAllTopicMappingsCorrectly1764410000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    console.log('🔧 Правильное исправление всех привязок тем к дисциплинам...');
    ...
    // TopicID тем из МХК (должны быть в Испанском, но сейчас в Испанском - значит правильно)
    const mhkTopicIds = [
    ...
    // TopicID тем из Культурологии (должны быть в МХК, сейчас в МХК - правильно)
    ...
    // TopicID тем из Психологии (должны быть в Культурологии, сейчас в Культурологии - правильно)
    ...
    // TopicID тем из Философии (должны быть в Психологии, сейчас в Психологии - правильно)
    ...
    // TopicID тем из Испанского (должны быть в Философии, сейчас в Философии - правильно)
    ...
    // Правильная циклическая перестановка:
    // Испанский ← МХК (темы МХК сейчас в Испанском, нужно переместить в МХК, а потом МХК темы в Испанский)
    // МХК ← Культурология (темы Культурологии сейчас в МХК, нужно переместить в МХК - но они уже там!)
    // Культурология ← Психология (темы Психологии сейчас в Культурологии, нужно переместить в Культурологию - но они уже там!)
    // Психология ← Философия (темы Философии сейчас в Психологии, нужно переместить в Психологию - но они уже там!)
    // Философия ← Испанский (темы Испанского сейчас в Философии, нужно переместить в Философию - но они уже там!)

    // Проблема: темы уже перемещены, но не в те дисциплины, куда нужно!
    // Нужно сделать правильную перестановку:
    // 1. Темы МХК (сейчас в Испанском) → остаются в Испанском (правильно)
    // 2. Темы Культурологии (сейчас в МХК) → остаются в МХК (правильно)
    // 3. Темы Психологии (сейчас в Культурологии) → остаются в Культурологии (правильно)
    // 4. Темы Философии (сейчас в Психологии) → остаются в Психологии (правильно)
    // 5. Темы Испанского (сейчас в Философии) → остаются в Философии (правильно)

    // НО! Пользователь сказал:
    // - "Мировая художественная культура — темы неправильные, они должны быть в Испанском языке" - значит темы МХК должны быть в Испанском (сейчас так и есть)
    // - "Культурология — темы неправильные, они должны быть в мировой художественной культуре" - значит темы Культурологии должны быть в МХК (сейчас так и есть)
    // - "Философия — эти темы должны быть в психологии" - значит темы Философии должны быть в Психологии (сейчас так и есть)
    // - "Психология — эти темы должны быть в культурологии" - значит темы Психологии должны быть в Культурологии (сейчас так и есть)

    // Значит всё правильно! Но пользователь говорит, что темы философии всё ещё в Психологии.
    // Значит нужно переместить темы Философии ИЗ Психологии В Философию, а темы Испанского ИЗ Философии В Испанский.

    // Правильная перестановка:
    // 1. Темы Испанского (сейчас в Философии) → Испанский
    ...
    // 2. Темы МХК (сейчас в Испанском) → МХК
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 3. Темы Культурологии (сейчас в МХК) → остаются в МХК (но должны быть в МХК по требованию пользователя)
    // НЕТ! Пользователь сказал "Культурология — темы неправильные, они должны быть в мировой художественной культуре"
    // Значит темы Культурологии должны быть в МХК (сейчас так и есть - правильно)

    // 4. Темы Психологии (сейчас в Культурологии) → остаются в Культурологии (правильно)

    // 5. Темы Философии (сейчас в Психологии) → Философия
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 6. Темы Психологии (сейчас в Культурологии) → Психология
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 7. Темы Культурологии (сейчас в МХК) → Культурология
    await queryRunner.query(`
      UPDATE "topics"
    ...
    // 8. Темы МХК (сейчас в МХК после шага 2) → Испанский
    await queryRunner.query(`
      UPDATE "topics"
      SET "DisciplineID" = '${SPANISH_DISCIPLINE_ID}'
      WHERE "TopicID" IN (${mhkTopicIds.map(id => `'${id}'`).join(', ')})
