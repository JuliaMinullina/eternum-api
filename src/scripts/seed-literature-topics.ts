...
async function bootstrap() {
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `⚠️  В базе уже есть ${existingCount} тем по литературе. Очищаю темы для этой дисциплины...`,
      );
    ...
    console.log(
      `✅ Успешно создано ${createdTopics.length} тем по литературе:`,
    );
    ...
    createdTopics.forEach((topic, index) => {
      ...
      // Категоризация тем
      if (
        topicName.includes('искусство слова') ||
        topicName.includes('роды и жанры') ||
        topicName.includes('направления') ||
        topicName.includes('фольклор')
      ) {
        categorizedTopics['Основы и теория'].push(topicName);
      } else if (
        topicName.includes('век') ||
        topicName.includes('русская') ||
        topicName.includes('советская') ||
        topicName.includes('древнерусская')
      ) {
        categorizedTopics['История русской литературы'].push(topicName);
      } else if (
        topicName.includes('Пушкин') ||
        topicName.includes('Лермонтов') ||
        topicName.includes('Гоголь') ||
        topicName.includes('Тургенев') ||
        topicName.includes('Толстой') ||
        topicName.includes('Достоевский') ||
        topicName.includes('Чехов') ||
        topicName.includes('Блок') ||
        topicName.includes('Ахматова') ||
        topicName.includes('Маяковский')
      ) {
        categorizedTopics['Классики и авторы'].push(topicName);
      } else if (
        topicName.includes('анализ') ||
        topicName.includes('методы') ||
        topicName.includes('стиховедение') ||
        topicName.includes('тропы') ||
        topicName.includes('композиция') ||
        topicName.includes('интертекстуальность')
      ) {
